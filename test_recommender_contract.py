"""
This file tests whether the app is running without errors or not
"""
...
def main():
    ...
    def has_required_keys():
        assert len(result) > 0, (
            "result list is empty, can't check keys (is that expected?)"
        )
    ...
    def no_duplicates():
        ...
        assert len(track_ids) == len(set(track_ids)), (
            "duplicate track_ids in one recommendation list"
        )
    ...
    def invalid_user_handled():
        ...
        raise AssertionError(
            "expected an error for an unknown user_id, but no error was raised"
        )

    check(
        "invalid user_id raises ValueError, not a silent failure", invalid_user_handled
    )
    ...
    check(
        "very large top_n doesn't crash (just returns fewer/what's available)",
        large_top_n_handled,
    )

    # 9. last user in the array works
